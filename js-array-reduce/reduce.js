...
function extractValue(arr, key) {
    return arr.reduce((overseer, valueIterator) =>{
        overseer.push(valueIterator[key]);
        return overseer;
    }, []);
}
...
appears in the string. This function should be case insensitive 
...
function addKeyAndValue(arr, key, value) {
    return arr.reduce((overseer, valueIterator, index) =>{
        overseer[index][key] = value;
        return overseer;
    }, arr);
}
...
in the array and if the result of the callback function 
...
function partition(arr, callback) {
    return arr.reduce((overseer, currentVal) =>{
        if(callback(currentVal)){
            overseer[0].push(currentVal);
        }else{
            overseer[1].push(currentVal);
        }
        return overseer; 
    }, [[],[]]);
}
